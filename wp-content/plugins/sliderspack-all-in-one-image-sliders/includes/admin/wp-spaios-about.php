...
function wp_spaios_register_about_page() {
	...
	add_submenu_page( 'edit.php?post_type='.WP_APAIOIS_POST_TYPE, __('Upgrade to PRO - SlidersPack', 'sliderspack-all-in-one-image-sliders'), '<span style="color:#2ECC71">'.__('Upgrade to PRO', 'sliderspack-all-in-one-image-sliders').'</span>', 'manage_options', 'wp-spaios-premium', 'wp_spaios_premium_page' );
}
...
function wp_spaios_about_page() {
	...
	<div id="fb-root"></div>
		...
		<div class="about-text"><?php echo sprintf( __('Thank you for using <a href="%s" target="_blank">%s</a>. Please <a href="%s" target="_blank">rate us</a>. A huge thanks in advance!', 'sliderspack-all-in-one-image-sliders'), 'https://www.wponlinesupport.com/', 'SlidersPack', 'https://wordpress.org/support/plugin/sliderspack-all-in-one-image-sliders/reviews/#new-post' ); ?></br></br>
